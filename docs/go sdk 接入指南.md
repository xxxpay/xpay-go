...
### SDK 简介
Go 语言的 SDK 基于 XPay 的 API 进行封装，以提供对于 Go 开发者而言更简单友好的接入方法。

目前 SDK 包含 payment、refund、redEnvelope、event、transfer 和 utils 模块。
* payment 模块封装了 payment 接口，可通过该接口发起交易。
...
获取 SDK ：
```go
go get -u github.com/xxxpay/xpay-go/xpay
```

在项目中导入 SDK：
```go
import "github.com/xxxpay/xpay-go/xpay"
```

在调用相应接口时还需要引入相应的模块，例如调用 payment 接口时，还需要导入：
```go
import "github.com/xxxpay/xpay-go/xpay/payment"
```
### Step2：设置 API KEY
在 XPay 的管理平台可以获得 Livekey 和 Testkey。
* 如果你没有填入参数或者是想在测试模式下发起交易。设置 Testkey。
* 如果已经填写了渠道参数，想发起真实交易，设置 Livekey。
```go
xpay.Key = "YOUR Test/Livekey"
```
...
#### 发起付款
##### 1. 获取支付凭据
payment 模块封装了 payment 接口，供发起交易请求使用。
payment.New 方法用于发起交易请求获取支付凭据。有两个返回值，第一个是支付凭据，也就是 Payment 对象，Payment 对象的字段介绍请看 [API 文档]()。第二个返回值是错误信息 error。
 ...
 *@param App              appid，XPay 分配的应用 ID。必填字段
...
// 由于 Go 语言 json 解析的特性，所有的参数的首字母都需要大写
  Order_no:   "123456789", 
  App:         xpay.App{Id: "app_PmfffHParTiD84aj"}, 
  Amount:      100, 
  Channel:     "alipay", 
  Currency:    "cny", 
  Client_ip:   "127.0.0.1", 
...
//返回的第一个参数 ch 是获取到的 Payment 对象，你需要将其转换成 json 给 Client ，或者 Client 接收后转换。第二个返回值 err 是错误信息，若正确返回 Payment 对象，则为 nil
ch, err := payment.New(params)
if err != nil {
  log.Fatal(err)
}
```

如果正常得到支付凭据，即 Payment 对象，示例如下：
```json
{
  ...
  "refunds": {
    "object": "list",
    "url": "/v1/payments/ch_Hm5uTSifDOuTy9iLeLPSurrD/refunds",
    "has_more": false,
    "data": []
  },
  "amount_refunded": 0,
  ...
  "credential": {
    "object": "credential",
    "alipay":{
      "orderInfo": "_input_charset=\"utf-8\"&body=\"tsttest\"&it_b_pay=\"1440m\"¬ify_url=\"https%3A%2F%2Fapi.pay.lucfish.com%2Fnotify%2Fpayments%2Fch_jH8uD0aLyzHG9Oiz5OKOeHu9\"&out_trade_no=\"1234dsf7uyttbj\"&partner=\"2008451959385940\"&payment_type=\"1\"&seller_id=\"2008451959385940\"&service=\"mobile.securitypay.pay\"&subject=\"test\"&total_fee=\"1.23\"&sign=\"dkxTeVhMMHV2dlRPNWl6WHI5cm56THVI\"&sign_type=\"RSA\""
    }
  },
  "description": null
}
```
##### 2. 将支付凭据返回给 Client 
付款需要将获得的支付凭据发送给 Client ， Client 接收支付凭据调起支付控件完成交易。具体传递需要商户根据自身处理方式去传递。由于 Go  SDK 获得的是 Payment 对象，需要将其转换成 JSON 再传给 Client 。
```go
...
```json
{
   ...
   "payment": "ch_Xsr7u35O3m1Gw4ed2ODmi4Lw"
}
```
...
用户在 Client 完成支付时，渠道会直接返回给 Client 支付结果。与此同时，XPay 会给商户指定的 Server 发送 Webhooks 通知。
通知以 POST 方式主动发送给商户 Server ，内容是 Event 事件。Event 事件的字段描述请参考 [API 文档]()。
商户需要以 Webhooks 通知作为支付成功与否的判断依据，不能使用 Client 的通知来判断支付结果。
#### 1. 配置 Webhooks
在 XPay 的平台上配置 Webhooks。商户将指定的 Server 地址配置到 Webhooks 里，同时还需要指定接收的事件类型，事件所属的支付环境（Test 环境/ Live 环境）等。如何配置 Webhooks 请参考 [Webhooks 使用指南]()。
Event 对象中的 type 字段表示事件类型。支付结果的事件类型是 payment.succeeded，该事件只在付款成功后才被发送；退款的事件类型是 refund.succeeded，同理只在退款成功后才会发送该事件。
商户需要选择接收这两个事件类型用来判断支付或退款是否成功。
#### 2. 获取 Webhooks 通知
支付成功时，XPay 主动以 POST 的方式发送 Webhooks 通知给商户指定的 Server 地址。商户需要在该地址监听并接收 Webhooks 通知。
* 接收到 Webhooks 通知后需要返回给 XPay  Server 状态 2xx(建议 200)。否则，建议返回状态码 500 表示未收到。
* 若商户 Server 未正确返回状态码 2xx，XPay 会在 25 小时内向商户 Server 最多发送 8 次 Webhooks 通知。时间间隔分别为 2min、10min、10min、1h、2h、6h、15h，直到用户向 XPay 返回 Server 状态 2xx 或者超过最大重发次数为止。
...
func webhook(w http.ResponseWriter, r *http.Request) {
  if r.Method == "POST" {
    ...
    buf.ReadFrom(r.Body)
    signature := r.Header.Get("x-xxxpay-signature") //从 header 获取签名字段
    webhook, err := xpay.ParseWebhooks(buf.Bytes()) // 调用parseWebhooks 方法解析 event 对象 
    if err != nil {
      fmt.Fprintf(w, "fail")
    } else {
      if webhook.Type == "payment.succeeded" {//由于 Go 解析 json 的特性，Event 对象中所有字段在取的时候首字母都需大写
...
**附**：类型为 payment.succeeded 的事件示例如下：
```json
{
    ...
    "data": {
        "object": {
            ...
            "refunds": {
                "object": "list",
                "url": "/v1/payments/ch_L8qn10mLmr1GS8e5OODmHaL4/refunds",
                "has_more": false,
                "data": []
            },
            "amount_refunded": 0,
...
```
需要注意的是 event 的 data 字段中包含了事件类型所属的对象，例如 payment.succeeded 中 data 里是 Payment 对象。其 key 是 object 字段。商户接收到即该事件即表明付款成功，也可从 payment 对象中判断 paid 字段是否为 true 来确定交易是否成功。
...
#### 验证 Webhooks 通知
XPay 对 Webhooks 通知进行了 RSA 签名，商户可使用签名验证 Webhooks 通知的合法性。
* 签名放在 Header 的自定义字段 x-xxxpay-signature 中。
* 签名用 RSA 私钥对 Webhooks 通知使用 RSA-SHA256 算法进行签名，以 base64 格式输出。

需要注意的是，在 Test 环境（用 Testkey 发起交易）不会生成签名。可以从 Header 中取到 x-xxxpay-signature 字段，但是值为空。只有在 Live 环境（用Livekey 发起交易）才会生成签名。

##### 验证签名
XPay 在管理平台中提供了 RSA 公钥，供验证签名。公司签约完成后，公钥信息在管理平台的「公司信息」中获取。验证签名需要以下步：
1. 从 header 取出签名并对其进行 base64 解码
```go
signature := r.Header.Get("x-xxxpay-signature")
```
2. 把 Webhooks 通知、XPay 管理平台提供的 RSA 公钥、 和 base64 解码后的签名三者一同放入 RSA 的签名函数中进行非对称的签名运算,来判断签名是否验证通过。

由于某些原因，验证签名的方法并没有集成在 SDK 里，但是我们提供了 Demo ，具体参考[Go SDK Demo](https://github.com/PingPlusPlus/xpay-go/blob/master/verifyDemo.go)。
