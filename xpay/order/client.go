...
func Payment(orderID, paymentID string) (*xpay.Payment, error) {
	return getC().Payment(orderID, paymentID)
}

func (c Client) Payment(orderID, paymentID string) (*xpay.Payment, error) {
	payment := &xpay.Payment{}

	err := c.B.Call("GET", fmt.Sprintf("/orders/%s/payments/%s", orderID, paymentID), c.Key, nil, nil, payment)
	return payment, err
}
...
func (c Client) PaymentList(orderID string, params *xpay.PagingParams) (*xpay.PaymentList, error) {
	...
	paymentList := &xpay.PaymentList{}
	err := c.B.Call("GET", fmt.Sprintf("/orders/%s/payments", orderID), c.Key, body, nil, paymentList)
