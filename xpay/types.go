// Package xpay golang sdk 数据类型定义
// types涵盖了以下数据格式
//    1.支付订单对象Payment;
//    2.订单退款对象Refund;
//    3.红包对象RedEnvelope;
...
type (
	...
	Refund struct {
		...
		Payment_order_no string                 `json:"payment_order_no"`
		Transaction_no   string                 `json:"transaction_no"`
		Funding_source   string                 `json:"funding_source,omitempty"`
		Extra            map[string]interface{} `json:"extra"`
	}
...
type (
	...
	OrderPayParams struct {
		Payment_amount   *int64                 `json:"payment_amount,omitempty"`
		Channel          string                 `json:"channel,omitempty"`
		Extra            map[string]interface{} `json:"extra,omitempty"`
		TimeExpire       int64                  `json:"time_expire,omitempty"` // 时间戳
		Payment_order_no string                 `json:"payment_order_no,omitempty"`
	}
	...
	Order struct {
		...
		Payment_essentials map[string]interface{} `json:"payment_essentials"`
		Available_balance  int64                  `json:"available_balance"`
...
type (
	...
	OrderRefund struct {
		...
		Payment_amount      int64                  `json:"payment_amount"`
		Balance_transaction string                 `json:"balance_transaction"`
		Payment_refund      string                 `json:"payment_refund"`
		Created             int64                  `json:"created"`
		Status              string                 `json:"status"`
...
type (
	...
	SplitProfit struct {
		ID            string                 `json:"id"`             // split_profit 对象 ID，由 XPay 生成。
...
type (
	...
	ProfitTransaction struct {
		ID            string `json:"id"`             // profit_transaction 对象 ID，由 XPay 生成。
...
type (
	...
	SplitReceiver struct {
		ID       string                 `json:"id"`       // split_receiver 对象 ID，由 XPay 生成。
