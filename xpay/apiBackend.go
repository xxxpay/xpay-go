...
func (s *ApiBackend) NewRequest(method, path, key, contentType string, body io.Reader, params []byte) (*http.Request, error) {
	...
	req.Header.Set("XPay-Request-Timestamp", requestTime)
	...
	if len(AccountPrivateKey) > 0 {
		...
		req.Header.Add("XPay-Signature", encodeSign)
	}

	// 添加Auth参数(获取参见 https://pay.lucfish.com/guidance/)满足 xpay api的http BasicAuth验证
