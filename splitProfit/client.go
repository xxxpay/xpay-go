...
// | charge | string |  | optional | 无 | xpay 交易成功的 charge ID
...
func List(app, charge, typ, channel string, params *xpay.PagingParams) (xpay.SplitProfitList, error) {
	return getC().List(app, charge, typ, channel, params)
}

// List 查询分账列表
func (c Client) List(app, charge, typ, channel string, params *xpay.PagingParams) (xpay.SplitProfitList, error) {
	values := &url.Values{}
	values.Add("app", app)
	if charge != "" {
