...
import (
	...
	// utils "xpay/xpay/utils"
	"strconv"
	"strings"
	"time"
)

var payServerDemo = new(PayServerDemo)

type PayServerDemo struct {
	demoAppID string
}

func (c *PayServerDemo) Setup(app string) {
	c.demoAppID = app
}

func (c *PayServerDemo) Run() {
...
func pay(w http.ResponseWriter, r *http.Request) {
	if strings.ToUpper(r.Method) == "GET" {
		...
		var paymentParams xpay.PaymentParams
		...
		json.Unmarshal(buf.Bytes(), &paymentParams)
		r := rand.New(rand.NewSource(time.Now().UnixNano()))
		orderno := r.Intn(999999999999999)
		extra := make(map[string]interface{})
		switch strings.ToLower(paymentParams.Channel) {
		case "upacp_wap":
