...
import (
	...
	"github.com/xxxpay/xpay-go/xpay/batchTransfer"
)
...
func (c *BatchTransfer) Unionpay_new() (*xpay.BatchTransfer, error) {
	recipients := []xpay.BatchTransferRecipient{
	...
	params := &xpay.BatchTransferParams{
		App:         c.demoAppID,
		Amount:      8000,
...
func (c *BatchTransfer) Alipay_new() (*xpay.BatchTransfer, error) {
	recipients := []xpay.BatchTransferRecipient{
...
func (c *BatchTransfer) Wxpub_new() (*xpay.BatchTransfer, error) {
	recipients := []xpay.BatchTransferRecipient{
...
func (c *BatchTransfer) Allinpay_new() (*xpay.BatchTransfer, error) {
	recipients := []xpay.BatchTransferRecipient{
...
func (c *BatchTransfer) Get() (*xpay.BatchTransfer, error) {
	return batchTransfer.Get(c.demoBatchTransferID)
}

//查询 Batch transfer 对象列表
//更多查询参数可以参照此链接 https://pay.lucfish.com/api?language=Go#查询-batch-transfer-对象列表
func (c *BatchTransfer) List() (*xpay.BatchTransferlList, error) {
...
func (c *BatchTransfer) Cancel() (*xpay.BatchTransfer, error) {
	return batchTransfer.Cancel(c.demoBatchTransferID) // 批量转账对象id ，由 XPay 生成（必须是unionpay渠道）
}
