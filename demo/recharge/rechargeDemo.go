...
import (
	...
	"github.com/xxxpay/xpay-go/xpay/recharge"
)
...
func (c *RechargeDemo) New() (*xpay.Recharge, error) {
	//这里是随便设置的随机数作为订单号，仅作示例，该方法可能产生相同订单号，商户请自行生成，不要纠结该方法。
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	orderno := r.Intn(999999999999999)

	rechargeParams := &xpay.RechargeParams{
		User: "1477895856250",
		Payment: xpay.RechargePayment{
			Amount:  10,
			Channel: "alipay_qr",
		...
		BalanceBonus: xpay.RechargeBonus{
			Amount: 1,
		},
...
func (c *RechargeDemo) Get() (*xpay.Recharge, error) {
	return recharge.Get(c.demoAppID, c.demoRechargeID)
}

func (c *RechargeDemo) List() (*xpay.RechargeList, error) {
