...
func (c *WebhookDemo) Setup(app string) {
	c.demoAppID = app
}

func (c *WebhookDemo) Run() {
...
func webhook(w http.ResponseWriter, r *http.Request) {
	if strings.ToUpper(r.Method) == "POST" {
		...
		signed := r.Header.Get("XPay-Signature")
		...
		errs := xpay.Verify([]byte(data), publicKey, decodeStr)
		if errs != nil {
			fmt.Println(errs)
			return
		} else {
		...
		webhook, err := xpay.ParseWebhooks(buf.Bytes())
