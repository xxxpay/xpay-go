...
import (
	...
	"github.com/xxxpay/xpay-go/xpay/balanceTransfer"
)
...
func (c *BalanceTransferDemo) New() (*xpay.BalanceTransfer, error) {
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	orderno := r.Intn(999999999999999)
	params := &xpay.BalanceTransferParams{
		Amount:      1,
		User:        "demoUser",
		Recipient:   "demoUser2",
...
func (c *BalanceTransferDemo) Get() (*xpay.BalanceTransfer, error) {
	return balanceTransfer.Get(c.demoAppID, c.demoTransferId)
}

//查询 BalanceTransfer 对象列表
func (c *BalanceTransferDemo) List() (*xpay.BalanceTransferList, error) {
